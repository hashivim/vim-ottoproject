...
# Use this script to update the commands auto-completed in plugin/otto.vim.

require 'open3'

command_re = /^\s\s\s\s(\S+)/
plugin_file = 'plugin/ottoproject.vim'

# Create the list of commands.
stdout, stderr, _status = Open3.capture3('otto list-commands')
output = if stderr == ''
           stdout.split("\n")
         else
           stderr.split("\n")
